...
# Stop signal listening port
STOP_PORT = 12000
stop_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
stop_socket.bind(("127.0.0.1", STOP_PORT))
stop_socket.settimeout(0.1)  # Set timeout to avoid blocking
...
try:
    while running:
        try:
            data, addr = stop_socket.recvfrom(1024)
            if data.decode().strip() == "STOP":
                print("🛑 Received stop signal, shutting down...")
                break
        except socket.timeout:
            pass  # No stop signal, continue execution
        except Exception as e:
            print(f"Stop socket error: {e}")
